...
void BankAccount::setBalance(double myBalance) {		//verifies that the balance is not negative
	if (myBalance >= 0) {
		balance = myBalance ;
	} else {
...
void BankAccount::setRate(double interest) {		//verifies that the interest rate is not set to a negative number
	if (interest >= 0) {
		rate = interest ;
	} else {
...
void BankAccount::setOwner(string name) {
	if(name.length() < 25) {		//owner's name must be less than 25 characters to be valid
		owner = name ;
	} else {
...
void BankAccount::setAccountNumber(int myAccountNumber) {		
	if (myAccountNumber >= 100000 && myAccountNumber < 1000000) {	//each account number is a 6-digit integer, verifies that the input matches this format
