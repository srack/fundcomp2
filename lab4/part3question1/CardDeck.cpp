...
CardDeck::CardDeck(int n) {
	srand(time(0)) ;		//seeds random number generator for shuffle function
	int i ;
	for( i = 0 ; i < n ; i++ ) {
...
int CardDeck::inOrder() {	//returns 1 if the CardDeck deck data member is in increasing order
	deque<int>::const_iterator forward ;
	for (forward = deck.begin() ; forward != deck.end() - 1 ; forward++) {
